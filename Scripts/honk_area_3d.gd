...
func interrupt_siren_song():
	if targeted_siren and targeted_siren.is_singing:
		...
		if master.is_coward and master.slaves.size() <= (master.MAX_SLAVES/2):
			ship.under_attack = false

			for slave in master.slaves:
				var opposed_dir = slave.global_position.direction_to(ship.position).normalized()
				slave.flee(-opposed_dir)
			var opposed_dir2 = master.global_position.direction_to(ship.position).normalized()
			master.flee(-opposed_dir2)
				
				
		elif master.slaves.size() == 0:
			ship.under_attack = false
			var opposed_dir = master.global_position.direction_to(ship.position).normalized()
